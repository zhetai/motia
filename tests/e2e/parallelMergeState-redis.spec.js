...
test.describe('Parallel Merge State Flow + Redis E2E', () => {
  ...
  test('verifies parallel merge flow state & events', async ({ page }) => {
    ...
    const startEvent = collectedEvents.find((ev) => ev.eventType === 'pms.start')
    const traceId = startEvent.metadata.flowTraceId
    ...
    // After flow completion, check that state was cleaned up
    ...
    const completionEvent = collectedEvents.find((ev) => ev.eventType === 'pms.join.complete')
