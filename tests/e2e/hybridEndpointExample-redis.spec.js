...
test.describe('Hybrid Flow E2E', () => {
  ...
  test('processes hybrid flow through all stages', async () => {
    ...
    const res = await fetch('http://localhost:4000/api/hybrid-endpoint-example', {
      method: 'POST',
    ...
    const validatedEvent = collectedEvents.find((e) => e.eventType === 'hybrid.validated')
    expect(validatedEvent).toBeDefined()
    expect(validatedEvent.data.items).toEqual(testData)

    const transformedEvent = collectedEvents.find((e) => e.eventType === 'hybrid.transformed')
    ...
    const enrichedEvent = collectedEvents.find((e) => e.eventType === 'hybrid.enriched')
    ...
    const completedEvent = collectedEvents.find((e) => e.eventType === 'hybrid.completed')
