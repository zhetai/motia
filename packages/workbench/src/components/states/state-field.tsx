...
const StateValue: React.FC<{ value: unknown }> = ({ value }) => {
        ...
        {Object.keys(valueObject).map((key) => (
          <div className="ml-8 flex flex-col" key={key}>
            ...
            <span className="text-large font-bold">
              {value ? (
                <StateValue value={valueObject[key as keyof typeof valueObject]} />
              ) : (
...
export const StateField = ({ label, value, className }: Props) => {
  return (
    <div className={cn('flex row gap-8 bg-gray-900 text-white p-4 rounded-lg font-bold text-sm', className)}>
      <div className="flex flex-col gap-1">
        ...
        <span className="text-large font-bold">{value ? <StateValue value={value} /> : JSON.stringify(value)}</span>
      </div>
    </div>
  )
}
