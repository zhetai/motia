...
The anatomy of a **Steps**, steps are broken down into the following components:
...
<DescriptionTable
  type={{
    ...
    description: {
      description: 'A small description of your step, this is not required, but it will be helpful for the workfbench flow visualization tool',
      type: 'string',
    }
  }}
/>
...
A **handler** function that processes an optional input and can **emit** new events, handlers are executed when a topic is emitted.
