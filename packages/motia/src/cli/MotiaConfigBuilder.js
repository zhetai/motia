...
// --------------- CORE BUILDER LOGIC ---------------
async function buildMotiaConfig(baseDir, configFilename) {
  ...
  console.log(`[MotiaConfigBuilder] Updated ${configFilename} in ${baseDir}`);
}

// --------------- CLI WRAPPER ---------------
async function main() {
  // Usage: node MotiaConfigBuilder.js <baseDir> <configFilename>
  // e.g. node MotiaConfigBuilder.js playground motia.config.json
  const baseDir = process.argv[2] || "playground"; // folder to scan
  const configFilename = process.argv[3] || "motia.config.json"; // config file name
  ...
  await buildMotiaConfig(absoluteBase, configFilename);
}
